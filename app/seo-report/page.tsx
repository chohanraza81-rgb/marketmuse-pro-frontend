...
import {
  ...
  LayoutDashboard, History, Settings, BarChart3, Zap
} from 'lucide-react';
...
export default function SEOReportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setProgressStep(0);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/seo-report`, {
        method: 'POST',
    ...
    // Note: we set submittingRef false after success? We don't need because page will redirect.
  };

  return (
    <main className="min-h-screen bg-[#0A0A0F] text-white font-['Inter'] relative overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-5xl h-[600px] bg-indigo-600/20 blur-[130px] pointer-events-none" />
      ...
      <nav className="sticky top-0 z-50 bg-black/70 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <Link href="/seo-report" className="px-4 py-2 text-sm text-white bg-indigo-600 hover:bg-indigo-500 transition-colors rounded-lg flex items-center gap-1.5">
      ...
      <section className="relative z-10 max-w-4xl mx-auto px-6 py-20">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md mb-6">
            ...
            <span className="text-xs uppercase tracking-widest text-neutral-300">SEO Intelligence Report</span>
          </div>
          ...
          <p className="mt-4 text-lg text-neutral-400 max-w-xl mx-auto">
            Enter a niche and target country. Our AI will analyze 50 keywords, SERP competitors, and local market insights.
          </p>
        </motion.div>
        ...
        >
          <div className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g., AI automation services Canada"
                  className="w-full pl-12 pr-4 py-3.5 rounded-xl bg-white/5 border border-white/10 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none text-white placeholder:text-neutral-500 transition-all"
                  required
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 font-bold text-white flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
        ...
        >
          ...
          >
            <div className="relative w-24 h-24 mx-auto mb-6">
              ...
              />
              <Sparkles size={40} className="absolute inset-0 m-auto text-indigo-400" />
            </div>
