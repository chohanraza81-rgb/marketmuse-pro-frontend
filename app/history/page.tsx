...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/reports?limit=50`)
      .then((r) => r.json())
  ...
  return (
    <main className="min-h-screen p-4 md:p-8 bg-[#020202] text-white">
      <h1 className="text-3xl font-bold bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] bg-clip-text text-transparent mb-6">
        Report History
      </h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {reports.map((report) => (
                <tr
                  key={report._id}
                  className="border-b border-[#1F1F1F]/50 hover:bg-white/5"
                >
                  <td className="p-2">
