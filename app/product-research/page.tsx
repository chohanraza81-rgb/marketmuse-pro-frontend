            ...
            {!loading ? (
              <motion.div key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 flex items-center justify-center"><Package size={22} className="text-emerald-400" /></div>
                  <div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="glass rounded-2xl p-6 space-y-4">
                    ...
                    <input type="text" value={niche} onChange={e => setNiche(e.target.value)} placeholder="e.g. wireless earbuds, yoga mats..." className="w-full bg-black border border-neutral-800 rounded-xl px-4 py-3.5 text-white placeholder-neutral-600 outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/20 transition-all text-lg" autoFocus required />
                  </div>
                  <div className="glass rounded-2xl p-6 space-y-4">
                      ...
                      {countries.map(c => (
                        <button key={c.code} type="button" onClick={() => setCountry(c.code)}
                          className={`flex flex-col items-center gap-1 p-3 rounded-xl border transition-all text-xs ${
                            country === c.code ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-neutral-800 hover:border-neutral-700 text-neutral-400'
                          }`}
                        >
                  ...
                  <button type="submit" className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold transition-all text-lg">Generate Product Report <ChevronRight size={18} /></button>
                </form>
              </motion.div>
            ) : (
              <motion.div key="loading" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass rounded-3xl p-12 text-center space-y-8">
                <div className="relative w-24 h-24 mx-auto">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center"><TrendingUp size={28} className="text-emerald-400" /></div>
                </div>
                <div><h2 className="text-xl font-semibold mb-2">Analyzing Market Data</h2><p className="text-neutral-500 text-sm">Scanning products, competitors & trends...</p></div>
