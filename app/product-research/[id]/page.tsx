...
export default function ProductReportPage() {
  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      <html>
        <head><title>MusePRO Report</title>
        <style>body{font-family:Arial;padding:40px;color:#000;line-height:1.6}pre{white-space:pre-wrap;font-size:12px}</style>
        </head>
        <body>
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `product-${report.niche}.csv`;
    a.click();
    ...
    return (
      <main className="min-h-screen bg-[#0A0A0A] flex items-center justify-center text-red-400">
        <div className="text-center">
          ...
          <Link href="/product-research" className="text-indigo-400">
            ← Back
          </Link>
        </div>
      </main>
  ...
  const demandTrend =
    data?.chart_data?.demand_forecast_12m?.map((v: number, i: number) => ({
      month: `M${i + 1}`,
      value: v,
    })) || [];
  const marketShare = data?.chart_data?.competitor_market_share || [];
  const profitBars =
    data?.pricing_engine?.map((p: any) => ({
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-white font-['Inter']">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="text-sm px-4 py-2 rounded-full bg-emerald-600 text-white font-medium"
            >
              Product
            </Link>
            <Link
              href="/seo-report"
              className="text-sm px-4 py-2 rounded-full bg-neutral-800 hover:bg-neutral-700 border border-neutral-700"
            >
              SEO
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex flex-wrap justify-between items-center gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-sm"
          >
      ...
      <div className="max-w-7xl mx-auto px-6 pb-10">
          ...
          <div className="p-4 rounded-xl bg-[#171717] border border-neutral-800">
            ...
            <div className="mt-2 w-full h-1.5 rounded-full bg-neutral-700">
              <div
                ...
                style={{ width: `${data?.market_score || 0}%` }}
              />
            </div>
          </div>
          <div className="p-4 rounded-xl bg-[#171717] border border-neutral-800">
            ...
            <p className="text-2xl font-bold font-mono">
              ${financials?.month6_profit_optimistic?.toLocaleString() || '-'}
            </p>
          </div>
          <div className="p-4 rounded-xl bg-[#171717] border border-neutral-800">
            <p className="text-xs text-neutral-400 mb-1">BREAKEVEN</p>
            <p className="text-2xl font-bold font-mono">
              {financials?.units_to_breakeven?.toLocaleString() || '-'} units
            </p>
          </div>
          <div className="p-4 rounded-xl bg-[#171717] border border-neutral-800">
            <p className="text-xs text-neutral-400 mb-1">TIME TO PROFIT</p>
            <p className="text-2xl font-bold font-mono">
              {financials?.months_to_profitability || '-'} months
            </p>
          </div>
        </div>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie
                    ...
                    paddingAngle={3}
                  >
                    {marketShare.map((_: any, idx: number) => (
        ...
        {/* Margin Comparison – New Progress‑Bar Table */}
        {profitBars.length > 0 && (
                  ...
                  <div className="flex-1 h-5 rounded-full bg-neutral-800 overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        background: `linear-gradient(90deg, #10B981 ${item.margin}%, #6366F1)`,
                      }}
                    />
                  </div>
